...
/* static const unsigned long psb[DB9_MAX_DEVICES][MAX_PORT_GPIO] = { { 1<<4, 1<<7, 1<<8, 1<<9, 1<<10, 1<<11, 1<<14 },
																...
																up down left right fire1 fire2 select */

/* Port gpio IDs */
static const unsigned char gpio_id[DB9_MAX_DEVICES][MAX_PORT_GPIO] = { { 6, 13, 19, 26, 16, 20, 21 },
																		{ 8, 17, 9, 22, 23, 24, 25 } };		
/* Port status bits */
static const unsigned long psb[DB9_MAX_DEVICES][MAX_PORT_GPIO] = { { 1<<6, 1<<13, 1<<19, 1<<26, 1<<16, 1<<20, 1<<21 },
																{ 1<<8, 1<<17, 1<<9, 1<<22, 1<<23, 1<<24, 1<<25 } };
...
static int __init db9_setup_pad(struct db9 *db9, int idx, int mode)
{
	...
	*(gpio+38) = 0x00;
	...
	printk("%s data pin connected to PORT%d\n", db9_mode->name, idx + 1);
